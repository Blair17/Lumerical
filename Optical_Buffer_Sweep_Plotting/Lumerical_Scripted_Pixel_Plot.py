...
def interpolate(x, y, array):
    ...
    X, Y = np.meshgrid(x, y)
    x_interp = np.linspace(700, 1000, 1000)
    y_interp = np.linspace(0, 200, 51)
...
files_refl = glob.glob('/Volumes/Sam/Lumerical/Thorpe_paper_simulations/test_structure/Alox_thickness_loops/data/refl/*.txt')
sorted_refl = sorted(files_refl, key = last_9chars)  

files_phase = glob.glob('/Volumes/Sam/Lumerical/Thorpe_paper_simulations/test_structure/Alox_thickness_loops/data/phase/*.txt')
...
opt_buffer_values = np.linspace(0, 200, 51)
...
plt.tight_layout()
plt.savefig('test_structure.png')