...
def interpolate(x, y, array):
    ...
    X, Y = np.meshgrid(x, y)
    x_interp = np.linspace(500, 1500, 1000)
    y_interp = np.linspace(0, 200, 101)
...
files_refl = glob.glob('/Volumes/Sam/Lumerical/Plasmonics/visible_metasurface_plasmonic/Au_Al2O3_ITO/Al2O3_Layer_Inv/thickness_Sweep/51/data/refl/*.txt')
sorted_refl = sorted(files_refl, key = last_9chars)  

files_phase = glob.glob('/Volumes/Sam/Lumerical/Plasmonics/visible_metasurface_plasmonic/Au_Al2O3_ITO/Al2O3_Layer_Inv/thickness_Sweep/51/data/phase/*.txt')
...
opt_buffer_values = np.linspace(0, 0.2, 101)
x_ticks = ['760', '770', '780', '790', '800', '810', '800']
x_ticks = ['760', '500', '700', '900', '1100', '1300', '1500']
...
plt.tight_layout()
plt.savefig('020124_Plasmonic_Thicknesss_Var_R_P.png')