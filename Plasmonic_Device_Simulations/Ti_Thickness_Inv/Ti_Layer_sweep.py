...
labels = ['0 nm', '1 nm', '2 nm', '3 nm', '4 nm', '5 nm']
...
for index, (l, p, i, k) in enumerate(zip(r_hundred, p_min, labels, range(colors))):
    ...
    # ax[0].set_ylim([0,100])
    ax[0].legend(loc='lower right', frameon=True, fontsize=13, framealpha=0, labelcolor='k')
    ...
    ax[1].legend(loc='center left', frameon=True, fontsize=13, framealpha=0, labelcolor='k')
...
plt.tight_layout()
plt.savefig('Ti_Inv.png', dpi=300)