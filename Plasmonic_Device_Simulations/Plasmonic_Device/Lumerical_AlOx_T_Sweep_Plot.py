...
x_tick_loc = ['']
x_ticks = [ '600', '500', '750', '1000', '1250', '1500']
...
for i, k in zip(rnge, color1):
    ax[0].plot(x, r[i][::-1], c=k, label=f'{opt_buffer_values[i].round(3)} nm')
    ...
    ax[1].plot(x, s[i][::-1], c=k, label=f'{opt_buffer_values[i].round(3)} nm')
...
plt.tight_layout()
plt.savefig('Plasmonic_Al2O3_Variation_plot.png')
# plt.show()